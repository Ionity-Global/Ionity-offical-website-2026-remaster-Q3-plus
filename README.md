...
| **🛰️ Edge Sensor Node** (Edge Lab, `edge.html`) | Turns *your* browser into a live edge node: real accelerometer, gyro/tilt/compass, ambient light and microphone sound-level — visualised live. The field-sensing pitch, demonstrated in your hand. |
| **🎛️ Interface sound** | Two restrained cues (a muted tap and a two-note confirm), **synthesised live** with the Web Audio API — zero audio files, no hover sounds, no ambient bed. Off by default; single toggle in the nav. |
